                ...
                <div className="w-full bg-varians-vr03 rounded-2xl hidden lg:block">
                  <div className="py-5 flex justify-around items-center">
                    <div className="flex items-center gap-2 text-primary-pr06">
                      ...
                      <p className="text-sm">{item ? formatDate(item.created_at) : "Loading..."}</p>
                    </div>
                    <p className="text-[#171717] font-medium w-[150px]">{item ? item.job_title || "Job Title" : "Loading..."}</p>
                    <div className="flex flex-col">
                      <div className="flex gap-2 text-xs text-[#171717] font-semibold">
                        <p>Average Score</p>
                        <p>{item ? (item.total_score == null ? 0 : item.total_score) : "Loading..."}</p>
                      </div>
                      <div className="mt-2 w-full bg-varians-vr00 rounded-3xl h-1">
                        <div
                          ...
                          style={{ width: `${item ? (item.total_score == null ? 0 : item.total_score) : 0}%` }}
                          role="progressbar"
                          aria-valuenow={item ? (item.total_score == null ? 0 : item.total_score) : 0}
                          aria-valuemin="0"
                  ...
                  <div className="py-3 px-4 flex justify-between items-center">
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="flex items-center gap-2 text-primary-pr06">
                        ...
                        <p className="text-xs">{item ? formatDate(item.created_at) : "Loading..."}</p>
                      </div>
                      <div className="flex flex-col">
                        <div className="flex gap-2 text-xs text-[#171717] font-semibold">
                          <p>Average Score</p>
